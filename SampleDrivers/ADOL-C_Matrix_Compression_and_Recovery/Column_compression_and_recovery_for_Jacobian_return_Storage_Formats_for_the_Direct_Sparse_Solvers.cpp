...
int main()
{
	...
	s_InputFile = baseDir + "mymatrix.mtx"; 
	...
	g->PartialDistanceTwoColoring("RIGHT_PARTIAL_DISTANCE_TWO", "SMALLEST_LAST");
	...
	*dp3_Seed = g->GetSeedMatrix(ip1_SeedRowCount, ip1_SeedColumnCount);
	...
	In stead of doing step 2.1-2.3 (Option 1), you can just call the bellow function:
		g->GenerateSeedJacobian(*uip3_SparsityPattern, rowCount, columnCount, dp3_Seed, ip1_SeedRowCount, ip1_SeedColumnCount, "RIGHT_PARTIAL_DISTANCE_TWO", "SMALLEST_LAST"); //Compress columns. This function is inside BipartiteGraphPartialColoringInterface class
	*/
	...
	*ip1_ColorCount = *ip1_SeedColumnCount;
	...
	JacobianRecovery1D::RecoverForPD2ColumnWise_SparseSolversFormat(g, *dp3_CompressedMatrix, *uip3_SparsityPattern, ip2_RowIndex, ip2_ColumnIndex, dp2_JacobianValue);
	...
	displayVector(*ip2_ColumnIndex, (*ip2_RowIndex)[g->GetRowVertexCount()]);
	cout<<"Display *dp2_JacobianValue"<<endl;
	displayVector(*dp2_JacobianValue, (*ip2_RowIndex)[g->GetRowVertexCount()]);
	Pause();
	return 0;
}
