...
namespace ColPack
{
	...
	int BipartiteGraphBicoloring::PreventLeftCycle(int i_LeftVertex, int i_DistanceOneVertex, int i_DistanceTwoVertex, int i_SetID, vector<int> & vi_CandidateColors, vector<int> & vi_FirstVisitToTreeOne, vector<int> & vi_FirstVisitToTreeTwo)
	{  
		int i_VertexOne, i_VertexTwo;

		if(i_SetID != _UNKNOWN)
		{
			i_VertexOne = vi_FirstVisitToTreeOne[i_SetID];
			i_VertexTwo = vi_FirstVisitToTreeTwo[i_SetID];
			
			if(i_VertexOne != i_LeftVertex)
			{
				vi_FirstVisitToTreeOne[i_SetID] = i_LeftVertex;
				vi_FirstVisitToTreeTwo[i_SetID] = i_DistanceOneVertex;
			}
			else
			if((i_VertexOne == i_LeftVertex) && (i_VertexTwo != i_DistanceOneVertex))    
			{	
				vi_CandidateColors[m_vi_LeftVertexColors[i_DistanceTwoVertex]] = i_LeftVertex;   

#if DEBUG == 3502

				cout<<"DEBUG 3502 | Acyclic Bicoloring | Probable Cycle Prevented | While Coloring Left Vertex "<<STEP_UP(i_LeftVertex)<<endl;
#endif
	...
	int BipartiteGraphBicoloring::PreventRightCycle(int i_RightVertex, int i_DistanceOneVertex, int i_DistanceTwoVertex, int i_SetID, vector<int> & vi_CandidateColors, vector<int> & vi_FirstVisitToTreeOne, vector<int> & vi_FirstVisitToTreeTwo)
	{
		int i_VertexOne, i_VertexTwo;
		...
		if(i_SetID != _UNKNOWN)
		{
			...
			if(i_VertexOne != i_RightVertex)
			{
				vi_FirstVisitToTreeOne[i_SetID] = i_RightVertex;
				vi_FirstVisitToTreeTwo[i_SetID] = i_DistanceOneVertex;
			}
			else
			if((i_VertexOne == i_RightVertex) && (i_VertexTwo != i_DistanceOneVertex))    
			{	
				vi_CandidateColors[m_vi_RightVertexColors[i_DistanceTwoVertex]] = i_RightVertex;   

#if DEBUG == 3503

				cout<<"DEBUG 3503 | Acyclic Bicoloring | Probable Cycle Prevented | While Coloring Right Vertex "<<STEP_UP(i_RightVertex)<<endl;
#endif
	...
	int BipartiteGraphBicoloring::UpdateLeftSet(int i_LeftVertex, int i_DistanceOneVertex, int i_DistanceTwoVertex, vector<int> & vi_FirstVisitToForestOne, vector<int> & vi_FirstVisitToForestTwo, vector<int> & vi_FirstVisitToForestThree)
	{
		int i_ColorID;

		int i_VertexOne, i_VertexTwo, i_VertexThree;

		i_ColorID = m_vi_RightVertexColors[i_DistanceOneVertex];
	    
		i_VertexOne = vi_FirstVisitToForestOne[i_ColorID];
		i_VertexTwo = vi_FirstVisitToForestTwo[i_ColorID];
		i_VertexThree = vi_FirstVisitToForestThree[i_ColorID];

		if(i_VertexOne != i_LeftVertex)
		{
			vi_FirstVisitToForestOne[i_ColorID] = i_LeftVertex;
			vi_FirstVisitToForestTwo[i_ColorID] = i_DistanceOneVertex;
			vi_FirstVisitToForestThree[i_ColorID] = i_DistanceTwoVertex;
		}
		else
		{
			return(m_mimi2_VertexEdgeMap[i_VertexThree][i_VertexTwo]);
		}
	...
	int BipartiteGraphBicoloring::UpdateRightSet(int i_RightVertex, int i_DistanceOneVertex, int i_DistanceTwoVertex, vector<int> & vi_FirstVisitToForestOne, vector<int> & vi_FirstVisitToForestTwo, vector<int> & vi_FirstVisitToForestThree)
	{
		int i_ColorID;

		int i_VertexOne, i_VertexTwo, i_VertexThree;

		i_ColorID = m_vi_LeftVertexColors[i_DistanceOneVertex];
		...
		if(i_VertexOne != i_RightVertex)
		{
			vi_FirstVisitToForestOne[i_ColorID] = i_RightVertex;
			vi_FirstVisitToForestTwo[i_ColorID] = i_DistanceOneVertex;
			vi_FirstVisitToForestThree[i_ColorID] = i_DistanceTwoVertex;
		}
		else
		{
			return(m_mimi2_VertexEdgeMap[i_VertexTwo][i_VertexThree]);
		}
...
//#define DEBUG asdf
	...
	/* OLD routines for Get...SeedMatrix()
	double** BipartiteGraphBicoloring::GetLeftSeedMatrix() {
		...
		//check and see if the coloring numbers start at 0
		for (int i=0; i < i_size; i++) {
			if(m_vi_LeftVertexColors[i]==0) {
				startAt1 = false;
				break;
			}
		}
		...
		else { // coloring numbers start at 0
			for (int i=0; i < i_size; i++) {
	...
	//BAD color 0 made out-of-bound
	double** BipartiteGraphBicoloring::GetRightSeedMatrix() {
		int i_size = m_vi_RightVertexColors.size();
		int i_num_of_colors = m_i_RightVertexColorCount;
		if(i_num_of_colors == 0 || i_size == 0) return NULL;

		int offset;
		if(m_i_SmallestRightVertexColorClass>0) { 
			//value of m_i_SmallestRightVertexColorClass has been calculated, use it for offset
			offset = m_i_SmallestRightVertexColorClass;
		}
		else { //find the value of offset on our own, should be equal to # of rows + 1
			 offset = m_vi_RightVertexColors[0];
			 for (int i=1; i<i_size; i++){
				 if(offset > m_vi_RightVertexColors[i]) offset = m_vi_RightVertexColors[i];
			 }
			 m_i_SmallestRightVertexColorClass = offset; //update m_i_SmallestRightVertexColorClass
		}
		...
		// populate Seed matrix
		for (int i=0; i < i_size; i++) {
			Seed[i][m_vi_RightVertexColors[i]-offset] = 1.;
		}
